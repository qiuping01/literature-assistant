  ...
  actions: {
    ...
    async deleteLiterature(id) {
      try {
        ...
        if (response.data.success) {
          // 删除成功后，从列表中移除该文献
          this.literatureList = this.literatureList.filter(item => item.id !== id)
          this.total = Math.max(0, this.total - 1)
          
          // 如果当前页没有数据了，且不是第一页，则回到上一页
          if (this.literatureList.length === 0 && this.queryParams.page > 1) {
        ...
        const errorMessage = error.response?.data?.message || error.message || '删除文献失败'
