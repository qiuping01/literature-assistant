...
public class LiteratureController {
    ...
    @Operation(summary = "预览文献文件", description = "在浏览器中预览文献文件（主要用于PDF）")
    public void previewLiteratureFile(
            @Parameter(description = "文献ID", required = true)
            @PathVariable @NotNull(message = "文献ID不能为空") Long id,
            jakarta.servlet.http.HttpServletResponse response) {

        log.info("预览文献文件，ID: {}", id);
        literatureService.previewLiteratureFile(id, response);
    }
    ...
    @Operation(summary = "获取文献文件本地路径", description = "获取文献文件的本地绝对路径，用于本地PDF阅读器打开")
    public Result<String> getLiteratureLocalPath(
            @Parameter(description = "文献ID", required = true)
            @PathVariable @NotNull(message = "文献ID不能为空") Long id) {

        log.info("获取文献文件本地路径，ID: {}", id);
        String localPath = literatureService.getLiteratureLocalPath(id);
     ...
     */
    @DeleteMapping("/{id}")
    @Operation(summary = "删除文献", description = "根据ID删除文献及其相关文件")
